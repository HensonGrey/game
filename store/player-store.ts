...
export const usePlayerStore = create<PlayerStore>()(
  persist(
    (set, get) => ({
      // Dev starts at a high-grade root for fast testing; real players start at Low Grade.
      spiritualRootIndex: IS_DEV ? 2 : 0,
      vitalityLevel: 0,
      originPoints: 50,
      lives: [],
      eternalInjuries: [],
    ...
    }),
    {
      ...
      // In dev, skip rehydration so a saved game can't overwrite the dev start
      // (boosted root). This is the reliable "wipe": the old save is never read.
      skipHydration: IS_DEV,
      partialize: (state) => ({
